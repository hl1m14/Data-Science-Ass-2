...
mapper = Code("""
        ...
        uniqueWords = function (words){
        	...
        	for (var j = 0; j < arrNewWords.length - 1; j++) {
				arrNewWords2.push(arrNewWords[j] +' '+arrNewWords[j+1]);
			}

			arrNewWords = arrNewWords2;
        	//^^^^^^
...
result = col.map_reduce(mapper, reducer, "bigram_result_count")


#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^step 3 : sort ^^^^^^^^^^^^^^^^^#

col = db.bigram_result_count
